...
function readJSExecutionStack(state: ReduxState) : JSContext[] {
  try {
    ...
    return execStack.values.map((addr) => {
      if (!addr.startsWith("#")) throw new Error("Invalid address in execution stack");
      const jsContext = state.irState.heap[addr];
      if (!jsContext) throw new Error("Invalid address in execution stack");
      if (jsContext.type !== "RecordObj") throw new Error("Invalid type in execution stack");
      ...
      if (f !== 'null') {
        ...
        if (!subMap || subMap.type !== 'MapObj') throw new Error("Invalid address in execution stack");

        const nameDescriptorAddr = subMap.map['"name"'];

        if (!nameDescriptorAddr) throw new Error("Invalid address in execution stack");

        const nameDescriptor = state.irState.heap[nameDescriptorAddr];

        if (!nameDescriptor || nameDescriptor.type !== 'RecordObj') throw new Error("Invalid address in execution stack");

        const nameWithQoute = nameDescriptor.map['Value'];
        if (!nameWithQoute) throw new Error("Invalid address in execution stack");
    ...
    toast.error('Failed to read JavaScript Execution Stack : is the program running?');
    return [];
  }
}
