...
export default function Address({ address, initialFold }: Props) {
  ...
  const [fold, setFold] = useState(
    initialFold === undefined ? false : initialFold,
  );
  const obj = useSelector((s: ReduxState) => s.irState.heap[address]);

  return (
    <div className="flex flex-col size-full">
      <span className="flex flex-row items-center justify-center break-all font-mono">
        {address}
        ...
        <CopyButton
          className="text-es-500 inline cursor-pointer hover:text-es-900 active:scale-50 transition-all"
          content={address}
        >
          <CopyIcon size={16} />
        </CopyButton>
        <a
          className="inline cursor-pointer text-es-500 hover:text-es-900 active:scale-50 transition-all"
          onClick={() => setFold(f => !f)}
        >
          {" "}
          {fold ? (
